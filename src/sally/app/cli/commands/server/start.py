...
def launch(args, expire=0.0):
    ...
    incept_args = inception_args(name=name, base=base, alias=alias, bran=bran, inceptFile=incept_file, config_dir=config_dir)
    ...
    if aeid is None:
        ...
        hab_cfg = dict()
        hab_cfg["salt"] = salt
        hby = habbing.Habery(name=name, base=base, bran=bran, cf=cf, **hab_cfg)
    else:
